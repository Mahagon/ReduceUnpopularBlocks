		...
		try {
			if(args.length == 2) {
				...
				if(config.getConfigurationSection("searchfor." + material) == null) {
					sender.sendMessage(ChatColor.RED + "[" + plugin.getName().toString() + "] " + material + " has already been deleted.");
					return true;
				}
				else {
					config.set("searchfor." + material, null);
					plugin.saveConfig();
					sender.sendMessage(ChatColor.GOLD + "[" + plugin.getName().toString() + "] " + material + " has been deleted.");
				}
			}
			else {
